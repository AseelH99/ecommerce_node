...
export const getProducts = async(req,res)=>{
    ...
    //const products = await mongooseQuery.sort(req.query.sort?.replace(',',' '));
    ...
    //  const products = await productModel.find(queryObj).limit(limit).skip(skip);
     ...
     return res.json({message:"success",total:count,products});
}
export const createProduct= async(req,res,next)=>{
    const{name,price,discount,categoryId,subcategoryId,description}=req.body;
    ...
    const {secure_url,public_id}= await cloudinary.uploader.upload(req.files.mainImage[0].path,{
        folder:`${process.env.APPNAME}/product/${req.body.name}/mainImages`
    });
    ...
    for(const file of req.files.subImages){
        const {secure_url,public_id}= await cloudinary.uploader.upload(file.path,{
            folder:`${process.env.APPNAME}/product/${req.body.name}/subImages`});
    ...
    if(!product){
        return res.status(400).json({message:"erroe while creating product"});
