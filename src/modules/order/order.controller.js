...
export const createOrder= async(req,res,next)=>{
  ...
  //  return res.status(200).json(req.body.products);
   if(couponName){
    ...
    if(coupon.expireDate<=currentDate){
        return next(new Error(`coupon is  expier`,{cause:400})); 
    }
    if(coupon.usedBy.includes(req.user._id)){
        return next(new Error(`coupon allready used`,{cause:409}));
    }
  ...
  for(let product of req.body.products){
    ...
    if(!checkProduct){
        return next(new Error(`product quantity not avalabil`));
    }
